...
from Scripts.dataset import create_dataset_for_ffn
from Scripts.models import MedicalQAModel
...
def multi_gpu_train(batch_size, num_gpu, data_path, num_epochs, model_path, learning_rate, loss=qa_pair_loss):
...
def single_gpu_train(batch_size, num_gpu, data_path, num_epochs, model_path, learning_rate, loss=qa_pair_loss):
    global_batch_size = batch_size*num_gpu
...
def train_ffn(model_path='/mnt/1T-5e7/mycodehtml/bio_health/NLP/Santosh_Gupta/ffn_cross_entropy/drive-download-20190429T033651Z-001',
    ...
    if num_gpu > 1:
        medical_qa_model = multi_gpu_train(
            batch_size, num_gpu, data_path, num_epochs, model_path, learning_rate, loss_fn)
    else:

...
class MedicalQAModelwithBert(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        if with_question:
            ...
            q_segment_ids=inputs['q_segment_ids']
            q_input_masks=inputs['q_input_masks']
            out_q_f_biobert=self.biobert((q_input_ids, q_segment_ids, q_input_masks))
            q_bert_embedding = out_q_f_biobert[self.layer_ind]
            
            q_bert_embedding = tf.reduce_mean(q_bert_embedding, axis=1)
        
        # ================================================================================
        if with_answer:
            a_input_ids=inputs['a_input_ids']
            a_segment_ids=inputs['a_segment_ids']
            a_input_masks=inputs['a_input_masks']
            out_a_f_biobert=self.biobert((a_input_ids, a_segment_ids, a_input_masks))
            a_bert_embedding = out_a_f_biobert[self.layer_ind]
